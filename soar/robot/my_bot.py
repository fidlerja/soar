...
#
# soar/robot/my_bot.py
""" A modified version of the PioneerRobot class,
    for representing a real or simulated Pioneer 3 robot.
...
from soar.sim.world import Polygon, Ray, Wall
...
class MyRobot(BaseRobot):
    ...
    def calc_sonars(self):  # Calculate the actual sonar ranges. Called once per simulated timestep
        ...
        for i in range(len(self.sonar_poses)):
            ...
            # print(collisions)
            target = self.world.find_all_collisions(sonar_ray, condition=self.is_color,eps=1e-3)
    ...
    def collision(self, other, eps=1e-8):
        ...
        if self.polygon.collision(other, eps=eps) and success:
            self._success = True
        elif isinstance(other, BaseRobot):  # Dispatch to collisions between robot polygons
